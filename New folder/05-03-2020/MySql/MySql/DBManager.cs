...
namespace MySql
{
    ...
    {
        private DBHandleFactory dbFactory;
        private IDBSwitch database;
        private string providerName;

        public DBManager(string connectionStringName)
        {
            dbFactory = new DBHandleFactory(connectionStringName);
        ...
        public long Insert(string commandText, CommandType commandType, IDbDataParameter[] parameters, string Eventout, long lastId)
        {
            lastId = 0;
            using (var connection = database.CreateConnection())
            {
